...
public class UsuarioController {
    ...
    @PostMapping("/{usuarioId}/favoritos/agregar/{restauranteId}")
    @PreAuthorize("#usuarioId == authentication.principal.usuario.id")
    @Operation(summary = "Agregar un favorito", description = "Añade un nuevo restaurante a la lista de favoritos del usuario correspondiente.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Guardado en la lista correctamente"),
            @ApiResponse(responseCode = "401", description = "No autorizado (Falta token)"),
            @ApiResponse(responseCode = "403", description = "Prohibido (Intento de modificar a otro usuario)")
    })
    public ResponseEntity<FavoritosResponseDTO> agregarFavorito(@PathVariable Long usuarioId, @PathVariable Long restauranteId) {
        return ResponseEntity.ok(usuarioService.agregarFavoritos(usuarioId, restauranteId));
    }

    @PostMapping("/{usuarioId}/favoritos/eliminar/{restauranteId}")
    @PreAuthorize("#usuarioId == authentication.principal.usuario.id")
    @Operation(summary = "Eliminar un favorito", description = "Elimina un restaurante de la lista de favoritos del usuario correspondiente.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Restaurante eliminado de la lista correctamente"),
            @ApiResponse(responseCode = "401", description = "No autorizado (Falta token)"),
            @ApiResponse(responseCode = "403", description = "Prohibido (Intento de modificar a otro usuario)")
    })
    public ResponseEntity<FavoritosResponseDTO> eliminarFavorito(@PathVariable Long usuarioId, @PathVariable Long restauranteId) {
        return ResponseEntity.ok(usuarioService.eliminarFavoritos(usuarioId, restauranteId));
    }

    @GetMapping("/{id}/favoritos")
    @PreAuthorize("#id == authentication.principal.usuario.id")
    @Operation(summary = "Listar favoritos", description = "Obtiene la lista completa de restaurantes favoritos del usuario logueado.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista obtenida correctamente"),
            @ApiResponse(responseCode = "401", description = "No autorizado (Falta token)"),
            @ApiResponse(responseCode = "403", description = "Prohibido (Intento de leer información de otro usuario)")
    })
