...
public class RestauranteController {
    ...
    @PostMapping("/{id}/platos/{platoId}/promocion/iniciar")
    @Operation(summary = "Iniciar Promocion")
    @PreAuthorize("#id == authentication.principal.restaurante.id")
    public ResponseEntity<Void> iniciarPromocion(
            @Valid @RequestBody PromocionRequestDTO dto,
            @PathVariable Long id,
            @PathVariable Long platoId) {

        platoService.realizarPromocion(id, dto, platoId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/{id}/platos/{platoId}/promocion/terminar")
    @Operation(summary = "Terminar Promocion")
    @PreAuthorize("#id == authentication.principal.restaurante.id")
    public ResponseEntity<Void> terminarPromocion(
            @PathVariable Long id,
            @PathVariable Long platoId) {

        platoService.terminarPromocion(id, platoId);
        return ResponseEntity.noContent().build();
    }
