...
public class RestauranteView {
    ...
    public RestauranteView(
        ...
        listaPlatos =
                new ListView<>();
        listaPlatos.setCellFactory(lv -> new ListCell<>() {

            private final Button btnIniciarPromo = new Button("Iniciar promo");
            private final Button btnTerminarPromo = new Button("Terminar promo");

            private final HBox botones = new HBox(10, btnIniciarPromo, btnTerminarPromo);

            @Override
            protected void updateItem(PlatoResponseDTO plato, boolean empty) {
                ...
                btnIniciarPromo.setOnAction(e -> {
                    try {
                        TextInputDialog input = new TextInputDialog();
                        input.setTitle("Promoción");
                        input.setHeaderText("Porcentaje de descuento");
                        ...
                        result.ifPresent(valor -> {
                            try {
                                ...
                                client.iniciarPromocion(idRestaurante, plato.getId(), dto);

                                cargarPlatos();

                            } catch (Exception ex) {
                                mostrarError("Error iniciando promoción");
                                ex.printStackTrace();
                            }
                        });

                    } catch (Exception ex) {
                        mostrarError("Error en promoción");
                    }
                });

                btnTerminarPromo.setOnAction(e -> {
                    try {
                        PlatoClient client = new PlatoClient();
                        client.terminarPromocion(idRestaurante, plato.getId());

                        cargarPlatos();

                    } catch (Exception ex) {
                        mostrarError("Error terminando promoción");
                        ex.printStackTrace();
                    }
                });
                ...
                VBox contenedor = new VBox(5, info, botones);
    ...
    private void crearPlato() {
        ...
        resultado.ifPresent(platoDTO -> {
            ...
            try {
                ...
                PlatoResponseDTO platoCreado =
                        client.crearPlato(idRestaurante, platoDTO);

                listaPlatos.getItems().add(platoCreado);
